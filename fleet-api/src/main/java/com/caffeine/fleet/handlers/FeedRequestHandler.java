...
public class FeedRequestHandler {
    ...
    public FeedRequestHandler handle(FleetRequest fleetRequest) {
        ...
        int state = Integer.parseInt(fleetRequest.getstate());

        /** ToDo: Remove this hard coding */
        String positioning = String.format("(%s,%s),null,null", lat, lon);
        LOG.info(String.format("%d -> %s", deId, positioning));
        LOG.info("{} -> {}", deId, positioning);
        LOG.info(deId + " -> " + positioning + ", state: " + state);
        ...
        DeMacros macros = getDeMacros(deliveryExecDetails);
